...
public class RlpRow extends TxnDataRow {
  ...
  public void traceRow(Trace.Txndata trace,  BlockSnapshot block) {
    ...
    trace
            ...
            .pRlpToAddressHi(txn.getSender().slice(0, 4).toLong())
            .pRlpToAddressLo(txn.getSender().slice(4, LLARGE))
            ...
            .pRlpGasPrice(besuTxn.getType().supports1559FeeMarket() ? Bytes.EMPTY : bigIntegerToBytes(besuTxn.getGasPrice().get().getAsBigInteger()))
            .pRlpMaxFeePerGas(besuTxn.getType().supports1559FeeMarket() ? bigIntegerToBytes(besuTxn.getMaxFeePerGas().get().getAsBigInteger()) : Bytes.EMPTY)
            .pRlpMaxPriorityFeePerGas(besuTxn.getType().supports1559FeeMarket() ? bigIntegerToBytes(besuTxn.getMaxPriorityFeePerGas().get().getAsBigInteger()) : Bytes.EMPTY)

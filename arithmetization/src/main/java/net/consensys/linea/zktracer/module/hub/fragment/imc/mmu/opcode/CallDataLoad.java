...
public class CallDataLoad extends MmuCall implements PostOpcodeDefer {
  ...
  public CallDataLoad(final Hub hub) {
    ...
    final long callDataSize = currentFrame.callData().range().size();
    final long callDataOffset = currentFrame.callData().range().offset();
    final EWord sourceOffset = EWord.of(currentFrame.frame().getStackItem(0));
    final long callDataCN = currentFrame.callData().callDataContextNumber();

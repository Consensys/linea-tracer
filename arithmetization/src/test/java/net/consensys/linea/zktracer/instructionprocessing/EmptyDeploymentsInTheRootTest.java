...
public class EmptyDeploymentsInTheRootTest {
    ...
    void deploymentTransactionLeadsToNonemptyDeploymentTest() {
        Transaction deploymentTransaction = deploymentTansactionFromInitCode(initCodeNonemptyDeployment);
        runTransaction(deploymentTransaction);
    }
    ...
    void createDeploysEmptyByteCode() {
        Transaction messageCallTransaction = messageCallTransactionToDeployerAccount(accountDeployerOfEmptyInitCode);
        runTransaction(messageCallTransaction);
    }

    @Test
    void createDeploysNonemptyByteCode() {
        Transaction messageCallTransaction = messageCallTransactionToDeployerAccount(accountDeployerOfNonemptyInitCode);
        runTransaction(messageCallTransaction);
    }

    @Test
    void createRevertsWithEmptyReturnData() {
        Transaction messageCallTransaction = messageCallTransactionToDeployerAccount(accountGeneratorOfRevertedCreateWithEmptyReturnData);
        runTransaction(messageCallTransaction);
    }

    @Test
    void createRevertsWithNonemptyReturnData() {
        Transaction messageCallTransaction = messageCallTransactionToDeployerAccount(accountGeneratorOfRevertedCreateWithNonemptyReturnData);
        runTransaction(messageCallTransaction);
    }
    ...
    ToyAccount accountDeployerOfEmptyInitCode =
            ToyAccount.builder().balance(Wei.fromEth(1)).nonce(13).address(Address.fromHexString("0x1337")).code(deployerOfEmptyDeploymentInitCode).build();
    ToyAccount accountDeployerOfNonemptyInitCode =
            ToyAccount.builder().balance(Wei.fromEth(1)).nonce(81).address(Address.fromHexString("0xadd7e550")).code(deployerOfNonemptyDeploymentInitCode).build();
    ToyAccount accountGeneratorOfRevertedCreateWithEmptyReturnData =
            ToyAccount.builder().balance(Wei.fromEth(1)).nonce(255).address(Address.fromHexString("0x69420")).code(deployerOfEmptyRevert).build();
    ToyAccount accountGeneratorOfRevertedCreateWithNonemptyReturnData =
            ToyAccount.builder().balance(Wei.fromEth(1)).nonce(1024).address(Address.fromHexString("0xdeadbeef")).code(deployerOfNonemptyRevert).build();

    List<ToyAccount> accounts = List.of(userAccount, accountDeployerOfEmptyInitCode, accountDeployerOfNonemptyInitCode, accountGeneratorOfRevertedCreateWithEmptyReturnData, accountGeneratorOfRevertedCreateWithNonemptyReturnData);

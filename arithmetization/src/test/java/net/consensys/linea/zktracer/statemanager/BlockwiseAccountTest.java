...
public class BlockwiseAccountTest {
  ...
  void testBlockwiseMapAccount() {
    ...
    // multiBlockEnv.getHub().transients().block();

    int blockCount = (multiBlockEnv.getHub().blockdata().getOperations().size() + 1) / 7;
        ...
        int relBlokNoFromBlock =
            (int)
                    (multiBlockEnv
                            ...
                            .get(blockCount + 6)
                            .relBlock()
                        - multiBlockEnv.getHub().blockdata().getFirstBlockNumber())
                + 1;

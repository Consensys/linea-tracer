    ...
    private void traceChunk(RomChunk chunk, int cfi, int cfiInfty, CompressedFileWriter writer) throws IOException {
        ...
        for (int i = 0; i < chunkRowSize; i++) {
            ...
            if (sliceNumber < nLimbSlice) {
                trace.counter(BigInteger.valueOf(i % LLARGE))
                        .counterMax(BigInteger.valueOf(LLARGE_MO));
                if (sliceNumber < nLimbSlice - 1) {
                    trace.nBytes(BigInteger.valueOf(LLARGE))
                            .nBytesAcc(BigInteger.valueOf((i % LLARGE) + 1));
                }
                if (sliceNumber == nLimbSlice - 1) {
                    trace
                            .nBytes(BigInteger.valueOf(nBytesLastRow))
                            .nBytesAcc(BigInteger.valueOf(nBytesLastRow).min(BigInteger.valueOf((i % LLARGE) + 1)));
                }

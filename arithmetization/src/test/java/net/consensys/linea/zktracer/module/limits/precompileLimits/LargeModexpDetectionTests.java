...
public class LargeModexpDetectionTests extends TracerTestBase {

  @ParameterizedTest
  @MethodSource("modexpInput")
  void modexpCall(int bbs, int ebs, int mbs) {
    // sender account
    ...
    final Map<String, Integer> lineCountMap = toyWorld.getZkTracer().getModulesLineCount();

    // check MODEXP limits:
    assertEquals(
        (bbs <= 512 && ebs <= 512 && mbs <= 512) ? 1 : Integer.MAX_VALUE,
        lineCountMap.get("PRECOMPILE_MODEXP_EFFECTIVE_CALLS"));
    final int maxByteWidth = Math.max(Math.max(bbs, ebs), mbs);
    assertEquals(
        (maxByteWidth > 32 && maxByteWidth <= 512) ? 1 : 0,
        lineCountMap.get("PRECOMPILE_LARGE_MODEXP_EFFECTIVE_CALLS"));
  }
  ...
  private static List<Integer> BYTE_SIZE_TO_TEST = List.of(0, 18, 32, 216, 318, 512);
}

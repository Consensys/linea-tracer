...
public class RefundTests extends TracerTestBase {
  ...
  void refundTest(Bytes callData, DominantCost dominantCostPrediction, TestInfo testInfo) {
    ...
    ToyExecutionEnvironmentV2 toyExecutionEnvironmentV2 = ToyExecutionEnvironmentV2.builder(chainConfig, testInfo)
    ...
    // Test blocks contain 5 transactions: 2 system transactions, 2 user transaction (the deployment transaction and the one we
  ...
  static Stream<Arguments> refundTestSource() {
    ...
    arguments.add(Arguments.of(Bytes.fromHexString("11".repeat(9)), DominantCost.EXECUTION_COST_DOMINATES));
    arguments.add(Arguments.of(Bytes.fromHexString("11".repeat(10)), DominantCost.FLOOR_COST_DOMINATES));

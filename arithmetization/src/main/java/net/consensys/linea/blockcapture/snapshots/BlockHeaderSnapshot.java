...
public record BlockHeaderSnapshot(
    ...
    Optional<String> withdrawalsRoot) {
  public static BlockHeaderSnapshot from(BlockHeader header) {
    return new BlockHeaderSnapshot(
        ...
        header.getWithdrawalsRoot().map(Bytes::toHexString));
  }
  ...
  public BlockHeader toBlockHeader() {
    ...
    // Following null checks are required for older replays only.  Eventually, it can be removed (i.e.
    // once all LONDON
    ...
    if (this.withdrawalsRoot != null) {
      this.withdrawalsRoot.ifPresent(
        root -> builder.withdrawalsRoot(Hash.fromHexString(root)));
    }

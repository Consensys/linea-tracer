...
public class RomLex implements Module {
  ...
  public void tracePreOpcode(MessageFrame frame) {
    ...
    final DeploymentInfo deploymentInfo = hub.conflation().deploymentInfo();
    ...
    switch (opcode) {
      ...
      case RETURN -> {
        ...
        if (!code.isEmpty() && depStatus) {
          codeIdentifierBeforeLexOrder += 1;
          Address address = frame.getContractAddress();
          int depNumber = deploymentInfo.number(address);
          ...
          this.addressRomChunkMap.put(address, chunk);
        }
      }
